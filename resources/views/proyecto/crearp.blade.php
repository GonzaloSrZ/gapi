...
<h4>Cargar Publicación</h4>
    ...
    <form action="/proyectos/guardar" x-data="data()" method="POST" class="formcarg">
        @csrf
        <div class="mb-3">
            <label for="" class="form-label">Titulo</label>
            <input id="titulo" required name="titulo" type="text" tabindex="1" class="form-control" placeholder="Ingrese titulo">
        </div>
        <div class="mb-3">
            <label for="" class="form-label">Descripción</label>
            <textarea id="descripcion" required name="descripcion" tabindex="2" class="form-control" placeholder="Ingrese descripción"></textarea>
        </div>
        <div class="mb-3">
...
{{--              <label for="" class="form-label">Fecha</label>
 ...
 --}}             <input type="hidden" value="{{auth()->user()->dni}}" id="dni" name="dni" >
            <input type="hidden" value="{{$proyecto->id}}" id="idproy" name="idproy" >
        </div>

        <a class="btn btn-secondary" href="/proyectos" role="button" tabindex="5">Cancelar</a>
        <button type="submit" class="btn btn-primary" tabindex="4">Confirmar</button>
